...
完成一次关联任务的专注后，可以在备注提示中点击“保存并完成 Todoist
任务”。本地 session 和待发送命令会先保存，网络成功后再显示为已同步；
断网时 Todoist 页会显示排队数量，恢复网络后打开菜单会自动重试。红色失败
状态表示 Todoist 明确拒绝了命令，可点击“重试”。完成成功后可点击“撤销
Todoist 完成操作”恢复任务。
