...
- 添加 XCTest Target 和计时核心的特征测试，锁定现有开始、停止、工作/休息切换行为。
- 建立 Debug、Release 命令行构建和测试基线，记录 Xcode 初学者的验证路径。
- 验收：Debug/Release 构建成功，测试能从 Xcode 和命令行运行，现有菜单栏行为无回归。

### M2：可测试的计时核心

- 引入 `TimerEngine`，逐步替代 UI 与状态机之间的直接耦合。
- 完成暂停/继续、停止、跳过、自动开始配置和五分之一番茄量化测试。
